...
\begin{document}	
	\noindent {\sffamily{\Huge{\textbf{Nikita Shubin}}}~\hfill~\href{https://github.com/NikitaShubin/}{{\sffamily{\Large{\textbf{\faGithub~GitHub}}}}}}
	\vspace{0.5em}

	\noindent +7 920 633 94 11 | \href{mailto:shubin.kit@ya.ru}{shubin.kit@ya.ru} | \faTelegram~\href{https://t.me/kitbrain}{@kitbrain} | Ryazan, Russia (relocation is acceptable)
	\vspace{0.5em}
	\hrule
	...
	\begin{tabular} {l | p{0.815\textwidth}}
		...
		\begin{itemize}
			\item Built a video data processing pipeline, assembled an annotation team, and developed software for speeding up annotation in \href{https://www.cvat.ai/}{CVAT} using Segment Anything Model \href{https://segment-anything.com/}{v1} and \href{https://sam2.metademolab.com/demo}{v2}. Key results presented at the \href{https://rutube.ru/video/4033ea0473199564892cc5a2f14dcfe6/}{D-Vision meetup}.
			\item Developed and trained models to solve assigned problems.
		\end{itemize}
\typeout{=== DEBUG-ENG-1: prevdepth=\the\prevdepth, baselineskip=\the\baselineskip, parskip=\the\parskip ===}
\\[0.5em]
		\textbf{2022 -- 2023} & Head of Machine Learning Team in <<ЭДС>> LLC:
		\begin{itemize}
			\item Object detection and segmentation on photos and videos (NDA).
			\item Reduced false positive rate by 70\% and miss rate by 15\% in binary segmentation.
			\item Detected and fixed a methodological error causing data leakage that inflated test metrics.
			\item Solved the problem of merging, parsing, and balancing three data sources in different formats.
			\item As Product Owner, led the development of a synthetic dataset generator using \href{https://www.unrealengine.com/en-US/unreal-engine-5}{Unreal Engine 5} to MVP.
		\end{itemize}
\typeout{=== DEBUG-ENG-2: prevdepth=\the\prevdepth, baselineskip=\the\baselineskip, parskip=\the\parskip ===}
\\[0.5em]
		...
		\end{itemize}
\typeout{=== DEBUG-ENG-3: prevdepth=\the\prevdepth, baselineskip=\the\baselineskip, parskip=\the\parskip ===}
\\[0.5em]
		...
		\end{itemize}
\typeout{=== DEBUG-ENG-4: prevdepth=\the\prevdepth, baselineskip=\the\baselineskip, parskip=\the\parskip ===}
\\[0.5em]
		...
		\end{itemize}
\typeout{=== DEBUG-ENG-5: prevdepth=\the\prevdepth, baselineskip=\the\baselineskip, parskip=\the\parskip ===}
	\end{tabular}
	\vspace{0.3em}\\
	\begin{longtable} {r | p{0.77\textwidth}}
		\textbf{Area of interest} & СV, ML, DL, multi-agent systems, embadded systems.\\
		
		\textbf{Research directions} & Object detection and parameters estimation algorithms: classical methods of image processing \cite{wrt}, neural network technologies application for programming the behavior of agents in a multi-agent system \cite{mas}. Efficient video data compression.\\
		
		\textbf{Basic skills} & \underline{Programming}: \it{Python, Matlab, Bash (Linux), Docker, CVAT, \LaTeX, Git.}\\ 
		& \underline{ML stack}: \it{Keras, TensorFlow, PyTorch, KerasTuner, ONNX, OpenCV, Scikit-learn, Scikit-image, Albumentations, Jupyter, Numpy, Numba, Pandas.}\\
		& \underline{Scientific research results publication}: \it{Participation in conferences, articles preparation.}\\
		...
		& \underline{English level:} \it{Intermediate.}\\
	\end{longtable}		
